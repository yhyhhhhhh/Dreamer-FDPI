...
from pwm_isaaclab_dfd_v3.compat import colorama
...
CostAwareActorCriticAgent = None
cfg_get = None
GdRiskCritic = None
DFDV2ReplayBuffer = None
DFDV2WorldModelWithContinuousCost = None
DualPolicyV3 = None
joint_train_dfd_v3 = None
...
def _load_training_deps():
    ...
    global gymnasium
    global CN
    global DreamerVecEnvWrapper
    global Logger
    global collect_training_info
    global make_unique_run_dir
    global save_run_artifacts
    global seed_np_torch
    global write_latest_run_pointer
    global CostAwareActorCriticAgent
    global cfg_get
    global GdRiskCritic
    global DFDV2ReplayBuffer
    global DFDV2WorldModelWithContinuousCost
    global DualPolicyV3
    global joint_train_dfd_v3
    ...
    from pwm_isaaclab_dfd_v2.agent_dfd_v2 import CostAwareActorCriticAgent as _CostAwareActorCriticAgent
    from pwm_isaaclab_dfd_v2.cost_utils import cfg_get as _cfg_get
    from pwm_isaaclab_dfd_v2.gd_risk import GdRiskCritic as _GdRiskCritic
    from pwm_isaaclab_dfd_v2.replay_buffer_dfd_v2 import DFDV2ReplayBuffer as _DFDV2ReplayBuffer
    ...
    from pwm_isaaclab_dfd_v3.dual_policy_v3 import DualPolicyV3 as _DualPolicyV3
    from pwm_isaaclab_dfd_v3.trainer_dfd_v3 import joint_train_dfd_v3 as _joint_train_dfd_v3
    ...
    CostAwareActorCriticAgent = _CostAwareActorCriticAgent
    cfg_get = _cfg_get
    GdRiskCritic = _GdRiskCritic
    DFDV2ReplayBuffer = _DFDV2ReplayBuffer
    DFDV2WorldModelWithContinuousCost = _DFDV2WorldModelWithContinuousCost
    DualPolicyV3 = _DualPolicyV3
    joint_train_dfd_v3 = _joint_train_dfd_v3
...
def _report_exception(exc, checkpoint_dir=None):
    ...
    print(colorama.Fore.RED + "\nDFD v3 training failed with exception:\n" + message + colorama.Style.RESET_ALL)
    if checkpoint_dir:
        try:
            os.makedirs(checkpoint_dir, exist_ok=True)
            error_path = os.path.join(checkpoint_dir, "dfd_v3_error.log")
            with open(error_path, "w", encoding="utf-8") as fout:
                fout.write(message)
            print(colorama.Fore.RED + f"Saved DFD v3 error report: {error_path}" + colorama.Style.RESET_ALL)
        except Exception as report_exc:
            print(colorama.Fore.YELLOW + f"Could not save DFD v3 error report: {report_exc}" + colorama.Style.RESET_ALL)


def load_dfd_v3_config(config_path):
    ...
    _set_default(dual_sampling, "MinModelUpdates", 1)
    _set_default(dual_sampling, "MinAgentUpdates", 1)
    _set_default(dual_sampling, "RequireGdReady", True)
    _set_default(dual_sampling, "MinGdUpdates", 1)
    _set_default(dual_sampling, "MinGdSeparation", 0.0)
    _set_default(dual_sampling, "RequireCoverageNeed", True)
    _set_default(dual_sampling, "MainCostRateThreshold", 0.10)
    _set_default(dual_sampling, "BoundaryRatioThreshold", 0.05)
    _set_default(dual_sampling, "CoverageWindowSteps", 6400)
    _set_default(dual_sampling, "BoundaryCostMin", 0.05)
    _set_default(dual_sampling, "BoundaryCostMax", 0.5)
...
def load_dfd_v3_initial_checkpoint(path, *, world_model=None, agent=None, map_location=None):
    ...
    if world_model is not None:
        ...
        if cost_head_keys and not has_v2_cost_head:
            ...
            print(
                colorama.Fore.YELLOW
                + "Skipped old checkpoint cost_head.* parameters because DFD v3 uses the continuous normalized cost head."
                + colorama.Style.RESET_ALL
            )
        ...
        if missing or unexpected:
            raise RuntimeError(
                "DFD v3 checkpoint load had incompatible non-cost-head keys: "
...
def main():
    ...
    conf = load_dfd_v3_config(args.config_path)
    ...
    save_run_artifacts(
        ...
        extra={
            ...
            "seed": args.seed,
            "device": args.device,
            "algorithm": "Dual-Imagination Continuous-Cost Dreamer v3",
    ...
    project = cfg_get(wandb_conf, "Project", "IsaacLab-PSSM-DFD-V3")
    run_group = cfg_get(wandb_conf, "Group", args.env_name)
    base_wandb_name = cfg_get(wandb_conf, "Name", f"DFD-v3-{args.env_name}-seed{args.seed}")
    ...
    if checkpoint_path:
        load_dfd_v3_initial_checkpoint(checkpoint_path, world_model=world_model, agent=agent, map_location=args.device)
        if hasattr(agent, "sync_slow_critic"):
            agent.sync_slow_critic()
        print(colorama.Fore.YELLOW + f"Loaded DFD v3 initial checkpoint: {checkpoint_path}" + colorama.Style.RESET_ALL)
    ...
    if bool(cfg_get(conf.DFD_V2.DualPolicy, "InitFromMainActor", True)):
        ...
        print(colorama.Fore.YELLOW + "Initialized DFD v3 dual actor from the main actor." + colorama.Style.RESET_ALL)
    ...
    try:
        ...
        print(colorama.Fore.YELLOW + "DFD v3 training interrupted by Ctrl-C." + colorama.Style.RESET_ALL)
        raise
