...
def _load_training_deps():
    ...
    from pwm_isaaclab_dfd_v4_lite.agent_fdpi_lite import FDPILiteActorCriticAgent as _FDPIRegimeActorCriticAgent
    from pwm_isaaclab_dfd_v4_lite.cost_utils import cfg_get as _cfg_get
    from pwm_isaaclab_dfd_v4_lite.dual_policy_v4 import DualPolicyV4 as _DualPolicyV4
    from pwm_isaaclab_dfd_v4_lite.replay_buffer_dfd_v4 import DFDV4ReplayBuffer as _DFDV4ReplayBuffer
    from pwm_isaaclab_dfd_v4_lite.risk_critics import GdRiskCriticV4 as _GdRiskCriticV4
    from pwm_isaaclab_dfd_v4_lite.risk_critics import GpRiskCritic as _GpRiskCritic
    from pwm_isaaclab_dfd_v4_lite.trainer_dfd_v4_lite import joint_train_dfd_v4 as _joint_train_dfd_v4
...
def load_dfd_v4_config(config_path):
    ...
    _set_default(main, "RampSteps", 300000)
    _set_default(main, "LambdaGp", 0.005)
    ...
    _set_default(checkpoint, "FullStatePrefix", "full_state_v4_lite")
...
def _report_exception(exc, checkpoint_dir=None):
    ...
    print(colorama.Fore.RED + "\nDFD v4-lite training failed with exception:\n" + message + colorama.Style.RESET_ALL)
    if checkpoint_dir:
        try:
            os.makedirs(checkpoint_dir, exist_ok=True)
            error_path = os.path.join(checkpoint_dir, "dfd_v4_lite_error.log")
            with open(error_path, "w", encoding="utf-8") as fout:
                fout.write(message)
            print(colorama.Fore.RED + f"Saved DFD v4-lite error report: {error_path}" + colorama.Style.RESET_ALL)
        except Exception as report_exc:
            print(colorama.Fore.YELLOW + f"Could not save DFD v4-lite error report: {report_exc}" + colorama.Style.RESET_ALL)
...
def _infer_step_from_full_checkpoint_path(path):
    ...
    suffix = ".pth"
    for prefix in ("full_state_v4_lite_", "full_state_v4_"):
...
def _load_v4_full_checkpoint(
    ...
    *,
    ...
    print(colorama.Fore.CYAN + f"Loaded DFD v4/v4-lite full checkpoint from {path}" + colorama.Style.RESET_ALL)
...
def main():
    ...
    parser.add_argument("--main_fdpi_ramp_steps", type=int, default=None)
    parser.add_argument("--main_fdpi_lambda_gp", type=float, default=None)
    ...
    if (
        ...
        or args.main_fdpi_ramp_steps is not None
        or args.main_fdpi_lambda_gp is not None
        ...
        if args.main_fdpi_ramp_steps is not None:
            conf.FDPIRegimeDreamer.MainFDPIRegime.RampSteps = int(args.main_fdpi_ramp_steps)
        if args.main_fdpi_lambda_gp is not None:
            conf.FDPIRegimeDreamer.MainFDPIRegime.LambdaGp = float(args.main_fdpi_lambda_gp)
        conf.freeze()
    ...
    if v4_full_checkpoint_path is None and v4_checkpoint_dir and v4_checkpoint_step is not None:
        for candidate_name in (
            f"full_state_v4_lite_{v4_checkpoint_step}.pth",
            f"full_state_v4_{v4_checkpoint_step}.pth",
        ):
            candidate = os.path.join(v4_checkpoint_dir, candidate_name)
    ...
    save_run_artifacts(
        ...
        extra={
            ...
            "seed": args.seed,
            "device": args.device,
            "algorithm": "FDPI-Lite Dreamer v4",
    ...
    project = cfg_get(wandb_conf, "Project", "IsaacLab-PSSM-DFD-V4-Lite")
    run_group = cfg_get(wandb_conf, "Group", args.env_name)
    base_wandb_name = cfg_get(wandb_conf, "Name", f"DFD-v4-lite-{args.env_name}-seed{args.seed}")
