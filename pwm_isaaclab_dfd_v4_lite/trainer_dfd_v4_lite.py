...
def _main_fdpi_cfg(fdpi_cfg):
    ...
    return {
        ...
        "StartStep": _cfg_int(main_cfg, "StartStep", 1500000),
        "RampSteps": _cfg_int(main_cfg, "RampSteps", 300000),
        "LambdaGp": _cfg_float(main_cfg, "LambdaGp", 0.005),
...
def train_agent_step_dfd_v4(
    ...
    *,
    ...
    if (
        ...
        update_fn = getattr(agent, "update_fdpi_lite", agent.update_fdpi_regime)
        info = update_fn(
            feat,
            action,
            discount,
        ...
        info["used_fdpi_lite"] = True
        return info
    agent.update(feat, action, discount, reward, weight, logger, step)
    if logger is not None:
        logger.log("MainFDPILite/enabled", 0.0, step)
    return {
        "used_fdpi_lite": False,
...
def _save_full_state_v4(
    ...
    *,
    ...
    payload = {
        "format": "fdpi_lite_dreamer_v4_full_state",
        "version": 1,
...
def joint_train_dfd_v4(
    ...
    print(colorama.Fore.CYAN + f"Saving DFD v4-lite checkpoints to {checkpoint_dir}" + colorama.Style.RESET_ALL)
    ...
    full_state_prefix = str(cfg_get(checkpoint_cfg, "FullStatePrefix", "full_state_v4_lite"))
    ...
    for iter_idx in tqdm(range(total_iters)):
        ...
        if iter_idx % save_every_iters == 0:
            print(colorama.Fore.GREEN + f"Saving DFD v4-lite model at total steps {env_steps}" + colorama.Style.RESET_ALL)
            ...
            if save_full_state:
                ...
                print(colorama.Fore.GREEN + f"Saved DFD v4-lite full state to {full_state_path}" + colorama.Style.RESET_ALL)
